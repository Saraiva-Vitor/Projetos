...
function y1 = Derivada_Primeira(x, x1) //x1 > Lambda + h x > Lambda
    h = 0.00001
    y1 = (f(x1) - f(x))/h
endfunction

function y2 = Derivada_Segunda(x, x1, x2) //x1 > Lambda + h, x2 > Lambda - h, x > Lambda
    h = 0.00001
...
function direcao = calc_direcao(x)
    //fxk = [(4*(x(1)-2)^3)+(2*(x(1)-2*x(2))), -4*(x(1)-2*x(2))]
    fxk = [(2*(x(1)-x(2)^3)+12*(x(1)-x(2))^3) , (6*x(2)*(x(1)-x(2)^3)+12*(x(1)-x(2))^3)]
...
function Solucao = Gradiente_Descendente(E, N, ponto_inicial)
    k = 1
    ponto = ponto_inicial
    Gradiente = [(4*(ponto_inicial(1)-2)^3)+(2*(ponto_inicial(1)-2*ponto_inicial(2))), -4*(ponto_inicial(1)-2*ponto_inicial(2))]
    Módulo_Gradiente = sqrt(Gradiente(1)^2 + Gradiente(2)^2)
    mprintf("k   \tXk                \tGradiente                \tMódulo         \tDireção                  \tLambda   \tXk+1\n")
    
    while ((Módulo_Gradiente >= E) & (k <= N))
        Direção = calc_direcao(ponto)
   
        Lambda = Newton(ponto, Direção)
        ponto = add_Lambda(ponto, Lambda, Direção)
        Gradiente = [(2*(ponto_inicial(1)-ponto_inicial(2)^3)+12*(ponto_inicial(1)-ponto_inicial(2))^3) , (6*ponto_inicial(2)*(ponto_inicial(1)-ponto_inicial(2)^3)+12*(ponto_inicial(1)-ponto_inicial(2))^3)]
        Módulo_Gradiente = sqrt(Gradiente(1)^2 + Gradiente(2)^2)
...
function main()
    ...
    Solucao = Gradiente_Descendente(E, N, x1)
    mprintf("\nSolução: (%g , %g)",Solucao);
endfunction
