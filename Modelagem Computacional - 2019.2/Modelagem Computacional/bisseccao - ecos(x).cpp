...
void get_info(double *intervalo_a, double *intervalo_b, double *erro, int *funcao){
	cout<<"Menu de op��es, escolha o componente a ser calculado:"<<endl;
...
double resistor(double x, vector<double>param){
	return (exp(-(0.005*x))*(cos(sqrt(2000-(0.01*(pow(x,2))))*0.05))-0.01);

}
// Calcula Indutancia
double indutor(double x, vector<double> param){
	return (exp(-(7/x))*(cos(sqrt((10000/x)-(19600/(pow(x,2))))*0.05))-0.01);
}
